...
| 2026-04-13 | v1.5.1 | **"제거" 맥락 잔존 표현 정리 + 입력 없음 처리**. v1.3 리프레이밍(⑯)에서 미수된 표현을 audit(=element) mode 전반에서 정리: Mode 표 "어떤 element가 안 쓰이나?" → "element 클릭률 랭킹은?", "강력한 제거 후보"/"제거 후보 판단" 표현을 "측정 기간 내 로그 없음"/"element 클릭률 랭킹의 주요 지표"로 교체. HTML 섹션 `removal-candidates` → `ranking-table` (상위 랭킹 강조, 하위는 중립 색). MD §5 "제거 후보" → "Element 클릭률 랭킹" 표. **Step 0 mode 추론 입력-없음 처리 신설** — `/funnel-check`만 호출해서 근거가 없을 때 자의적으로 mode를 고르지 않고, 표(flow/audit)와 예시 질문을 그대로 안내하고 사용자가 선택하도록. audit 예시 문구에 "안 쓰이는", "제거", "낭비" 등 가치판단 단어 금지 원칙 명시. **판단(유지/개선/리디자인/제거)은 사용자 몫, 스킬은 데이터 제공**을 여러 곳에 반복 명시 |
